...
class ChannelsController extends Controller
{
    ...
    public function changeData(Request $request, $id)
    {
        ...
        if($files=$request->file('datapdf')){  
            $name=$files->getClientOriginalName();  
            $files->move('data',"data.pdf");  
            $a = new PDF2Text();
            $a->setFilename('data/data.pdf'); 
            $a->decodePDF();
            $my = $a->output();
            $string = trim(preg_replace('/\s\s+/', ' ', $my));
          #  dd($string);
            $channel->data.=$string;
        }  
        if($channel->data==null){
            $channel->data="No data";
        }
        $channel->save();
        return redirect()->back();
    }
    ...
    {
        $channel = \App\Channel::select('name','id')->get();
        
        return $channel;
    }

    public function getQueryAnswer(Request $request, $id)
    {
        $channel = \App\Channel::where('id', $id)->get()[0];
        $data = $channel->data;
        $query = $request->get("query");
        $type= $channel->type;
        $mem = explode(",",$channel->members);
        if($type==1){
            if($request->get("email")== null || !(in_array($request->get("email"),$mem)))
            {
